...
esp_esph_hm_rf_eth = esph_hm_rf_eth_ns.class_("esph_hm", cg.Component)
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_CONNECTED): cv.use_id(binary_sensor.BinarySensor),
            cv.Optional(CONF_RADIO_MODULE_TYPE): cv.use_id(text_sensor.TextSensor),
            cv.Optional(CONF_FIRMWARE_VERSION): cv.use_id(text_sensor.TextSensor),
            cv.Optional(CONF_SERIAL): cv.use_id(text_sensor.TextSensor),
            cv.Optional(CONF_SGTIN): cv.use_id(text_sensor.TextSensor),
        }
    )
...
async def to_code(config):
    ...
    if CONF_CONNECTED in config:
        connected_sensor = await cg.get_variable(config[CONF_CONNECTED])
        cg.add(var.set_connected_sensor(connected_sensor))

    if CONF_RADIO_MODULE_TYPE in config:
        radio_module_type_sensor = await cg.get_variable(config[CONF_RADIO_MODULE_TYPE])
        cg.add(var.set_radio_module_sensor(radio_module_type_sensor))

    if CONF_FIRMWARE_VERSION in config:
        firmware_sensor = await cg.get_variable(config[CONF_FIRMWARE_VERSION])
        cg.add(var.set_firmware_sensor(firmware_sensor))

    if CONF_SERIAL in config:
        serial_sensor = await cg.get_variable(config[CONF_SERIAL])
        cg.add(var.set_serial_sensor(serial_sensor))

    if CONF_SGTIN in config:
        SGTIN_sensor = await cg.get_variable(config[CONF_SGTIN])
